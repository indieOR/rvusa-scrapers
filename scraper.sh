...
for classurl in `curl -s https://www.rvusa.com/rv-guide/specs-by-type-class-c-t3  | grep -o \/rv-guide\/specs-by-model.*t3`; do
#for classurl in `curl -s ${baseurl}/rv-guide/specs-by-type-class-a-t2 | grep -o \/rv-guide\/specs-by-model.*t2`; do
	...
	#https://www.rvusa.com/rv-guide/specs-by-model-coachmen-encore-m7714-b113
	for yearurl in `curl -s ${baseurl}/${classurl} | grep -Po "/rv-guide/specs-by-model-\d\d\d\d.*t3"`; do
		#At Specs_Guide->
		#Get list of years
		for floorplanurl in `curl -s ${baseurl}${yearurl} | grep -Po \/rv-guide\/.*floorplan.*\"\> | grep -v "h2 class" | awk -F '"' '{print $1}'`; do
			...
			./rvparse.py --url ${baseurl}${floorplanurl} --path ./classc/ &
		done
	done
done
